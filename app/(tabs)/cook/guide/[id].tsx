    ...
    try {
      ...
      if (Platform.OS === 'web') {
        try {
          ...
          console.log('[CookGuide] web printToFileAsync failed', webErr instanceof Error ? webErr.message : String(webErr));
        }
        try {
          await print.printAsync({ html });
        } catch (printErr) {
          console.log('[CookGuide] web printAsync failed', printErr instanceof Error ? printErr.message : String(printErr));
          await Share.share({ message: exportText, title: entry.title });
        }
        return;
      }
      ...
      if (fs && cacheDir) {
        ...
        try {
          ...
          finalUri = destUri;
          console.log('[CookGuide] moved PDF to', destUri);
        } catch (moveErr) {
          console.log('[CookGuide] move failed, using original', moveErr instanceof Error ? moveErr.message : String(moveErr));
        }
      }
...
const styles = StyleSheet.create({
  ...
  imageActionDanger: {
    backgroundColor: 'rgba(196,48,48,0.25)',
    borderColor: 'rgba(196,48,48,0.40)',
  },
  heroText: {
    padding: 16,
  },
  ...
  pill: {
    ...
    borderWidth: StyleSheet.hairlineWidth,
    borderColor: 'rgba(192,92,59,0.22)',
    justifyContent: 'center',
  },
  pillSecondary: {
