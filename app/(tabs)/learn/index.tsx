...
function buildOverviewFromRow(row: SupabasePlantRow): string | undefined {
  const lines: string[] = [];
  const season = typeof row.seasonality_note === 'string' ? row.seasonality_note.trim() : '';
  if (season) {
    lines.push(`Seasonality: ${season}`);
  }
  const prep = Array.isArray(row.prep_basics) ? row.prep_basics.map((p) => String(p)).filter(Boolean) : [];
  if (prep.length > 0) {
    lines.push(`Prep: ${prep.slice(0, 2).join('; ')}`);
  }
  const warnings = Array.isArray(row.warnings) ? row.warnings.map((w) => String(w)).filter(Boolean) : [];
...
function toLearnPlant(row: SupabasePlantRow, index: number): LearnPlant {
  ...
  return {
    ...
    category: row.primary_category ? titleCase(String(row.primary_category)) : undefined,
    overview: buildOverviewFromRow(row),
    isBushTucker: row.edibility_status ? row.edibility_status !== 'not_edible' : true,
    isMedicinal: false,
...
async function fetchPlantsFromSupabase(): Promise<LearnPlant[]> {
  ...
  try {
    ...
    const { data, error } = await supabase
      .from('plants')
      .select(
        'id, slug, common_name, scientific_name, primary_category, edible_parts, edibility_status, prep_basics, warnings, seasonality_note, ui_accent'
      )
