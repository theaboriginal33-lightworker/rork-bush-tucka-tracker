...
function buildOverviewFromRow(
  row: SupabasePlantRow,
  edibleParts: string[],
  prepBasics: string[]
): string | undefined {
  const lines: string[] = [];
  if (edibleParts.length > 0) {
    lines.push(`Edible parts: ${edibleParts.join(', ')}`);
  }
  const season = typeof row.seasonality_note === 'string' ? row.seasonality_note.trim() : '';
  if (season) {
    lines.push(`Seasonality: ${season}`);
  }
  if (prepBasics.length > 0) {
    lines.push(`Prep: ${prepBasics.slice(0, 2).join('; ')}`);
  }
  return lines.length > 0 ? lines.join('\n') : undefined;
}

function buildPreparationFromBasics(prepBasics: string[]): string | undefined {
  if (prepBasics.length === 0) return undefined;
  return `Steps:\n• ${prepBasics.join('\n• ')}`;
}

function buildWarningsFromRow(status: string | null | undefined, warnings: string[]): string | undefined {
  const lines: string[] = [];
  if (status) {
    const normalized = titleCase(status.replace(/_/g, ' ')).toUpperCase();
...
function buildTagsFromRow(category: string | undefined, status: string | null | undefined, edibleParts: string[]): string[] {
  const tags: string[] = [];
  if (category) tags.push(category);
  if (status) tags.push(`Safety: ${titleCase(status.replace(/_/g, ' '))}`);
  for (const part of edibleParts) {
    tags.push(`Part: ${titleCase(part)}`);
  }
  return tags;
}
...
const FALLBACK_PLANTS: LearnPlant[] = [
  ...
  },
  {
    ...
    category: 'Seed',
    overview:
      'Wattleseed comes from various native Acacia species found across Australia. Traditionally harvested and prepared by Aboriginal communities, it has been a reliable, nutrient-dense food source for thousands of years.\n\nWhen roasted and ground, wattleseed develops a rich, nutty flavour with notes of coffee, chocolate, and hazelnut.\n\nNutritional insight (general): traditionally valued for protein, complex carbohydrates, fibre, and sustained energy. Nutritional values vary by species and environment.',
    isBushTucker: true,
    isMedicinal: false,
    safetyLevel: 'caution',
    confidenceHint: 'Only certain Acacia species are edible. Roast seeds before use. If unsure of the species, do not consume.',
    edibleParts: ['seed'],
    preparation:
      'Collect dry, mature pods. Remove seeds. Roast thoroughly to improve flavour and safety. Grind into meal or use whole. Roasting enhances digestibility and aroma.',
    seasonality: 'Seed pods: Late Spring → Summer',
    seasonalityNote: 'Pods mature and dry on the tree before splitting open.',
    warnings:
      'Status: CAUTION — SPECIES & PREPARATION MATTER\n\n• Only certain Acacia species produce edible seeds\n• Seeds must be roasted before consumption\n• Raw seeds are not recommended\n• Proper identification is essential\n• If unsure of the species, do not consume',
    lookalikes: 'Other Acacia species produce inedible seeds; confirm pod and leaf form.',
    culturalNotes:
      'Traditionally harvested during dry seasons. Seeds were ground using stone tools and used as both staple and flavouring. Knowledge of edible species is passed through generations.\n\nWattleseed teaches patience — nourishment comes through preparation.\n\nRespect & sustainability:\n• Harvest mature pods only\n• Take small amounts\n• Leave trees healthy and intact\n• Respect local land access and custodianship\n\nApp guidance:\n• Gate recipes behind roasted confirmation\n• Prompt species verification in Tucka Guide\n• Suggest grinding for beginners\n• Highlight small-quantity use\n\nCultural knowledge shared here is general and non-restricted.',
    suggestedUses:
      'Ground seed flour • Baking & breads • Desserts • Beverages • Savoury spice blends\n\nOften used in small quantities to add depth and richness.',
    prepBasics: [
      'collect dry, mature pods',
      'remove seeds',
      'roast thoroughly',
      'grind into meal or use whole',
    ...
    tags: [
      ...
      'Safety: Caution',
      'Preparation: Required',
      'Flavour Profile: Nutty / Roasted',
      'Environment: Widespread / Woodland',
    ],
...
function toLearnPlant(row: SupabasePlantRow, index: number): LearnPlant {
  ...
  const category = row.primary_category ? titleCase(String(row.primary_category)) : undefined;
  const status = row.edibility_status ? String(row.edibility_status) : undefined;
  ...
  return {
    ...
    category,
    overview: buildOverviewFromRow(row, edibleParts, prepBasics),
    isBushTucker: status ? status !== 'not_edible' : true,
    isMedicinal: false,
    safetyLevel: status,
    edibleParts,
    preparation: buildPreparationFromBasics(prepBasics),
    seasonality: undefined,
    warnings: buildWarningsFromRow(status, warnings),
    lookalikes: undefined,
    culturalNotes: undefined,
    suggestedUses: undefined,
    prepBasics,
    seasonalityNote: row.seasonality_note ? String(row.seasonality_note) : undefined,
    edibilityStatus: status,
...
async function fetchPlantByIdOrSlug(idOrSlug: string): Promise<LearnPlant | null> {
  ...
  try {
    ...
    const { data, error } = await supabase
      .from('plants')
      .select(
        'id, slug, common_name, scientific_name, primary_category, edible_parts, edibility_status, prep_basics, warnings, seasonality_note, ui_accent, created_at, updated_at'
      )
